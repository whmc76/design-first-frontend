...
- **Surface graph:** scoped routes, views, modes, nested flows, overlays, return paths, and shared-shell relationships.
- **Route and viewport scope:** exact route/view/mode, desktop/mobile scope, target and boundary widths.
...
## 2. Experience Coverage Matrix

Use the schema and coverage gate from `experience-coverage.md`. For every coverage row, record:

- Stable coverage ID and governing artifact ID.
- Journey step, entry/return path, route/view/mode, state, and viewport/container.
- Shared shell and template family.
- The distinct design decision that requires this row or the reason it safely shares an artifact.
- Scope status and any blocker.

One attractive screen is not a contract for an uncovered multi-surface product.
...
List the evidence required for this problem and map each item to coverage IDs:

- Stable artifact ID, source path/URL, type, target viewport, coverage IDs, and approval/supersession state.
- System frame for shared shell, navigation, visual system, and cross-screen continuity when needed.
- Static screen artifacts for each structurally distinct template family.
- State storyboard or state references for critical transitions and recovery.
...
A contact sheet may index the set but cannot replace the separate full-resolution sources. If the entire scoped product is governed by one frame, record why the single-surface exception is valid.
...
## 8. Data, Action, and Feasibility Contract
...
- Cross-artifact invariants and any deliberate per-template variation.

## 10. Module-to-Code Map

```markdown
| Coverage/artifact | Contract item | Existing owner/selector | Action | Data/handler owner | States | Verification |
| --- | --- | --- | --- | --- | --- | --- |
| C-01 / A-01 | Elastic work shell | `WorkspaceShell`, `.workspace-*` | rewrite shared layout | presentation context | normal/focused/collapsed | DOM geometry at 1440/1920 |
| all / A-01 | Old fixed overlay | `.old-overlay` | remove | n/a | all | selector count 0 |
```
...
## 11. Preserve, Remove, and Deviate
...
- Every scoped surface-graph node has a coverage row and every row maps to approved evidence, `OUT_OF_SCOPE`, or an explicit blocker.
- The artifact set covers each distinct shell, template family, important nested flow, structurally different state, and scoped breakpoint without contradictory shared decisions.
...
Any `PLANNED`, unexplained, or uncovered row blocks the freeze. A `BLOCKED` row makes the design incomplete and must be reported as such.
